...
def main():
    ...
    if all(status_codes):
        ...
        if len(status_store.messages) == 4:

            subject = f'WebChecks OK: All {len(status_codes)} sites are UP!'
            print(f'Sending email to inform of {len(status_store.messages)} success checks so far...')

            email_body = ""

            for message in status_store.messages:
                email_body += "\n"
                email_body += message
            ...
            print(f'Not sending emails, {len(status_store.messages)} ok so far...')
    else:
