...
{
   fs.readFile('/sys/bus/w1/devices/28-0000045bf342/w1_slave', function(err, buffer)
   {
      if (err)
      {
         console.error(err);
      ...
      //console.log(now.toLocaleString() + ": temperature = " + temp);
      if (temp < 1.0)
      {
        console.log(now.toLocaleString() + ': ALERT: Skipped abnormal value: ' + temp);
...
var PORT = 8001;
var DB_LOG_INTERVAL_MINS = 5;

// Start temperature logging (every 5 min).
var msecs = (60 * DB_LOG_INTERVAL_MINS) * 1000; // log interval duration in milliseconds
logTemp(msecs);

// Send a message to console
console.log('Server is logging to database at ' + DB_LOG_INTERVAL_MINS + ' mins intervals');
